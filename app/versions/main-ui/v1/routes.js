...
router.get('/lists/', [handlers.configureCurrentUrl, handlers.configureNavigation, function (req, res) {
  res.render('cards', {
    ...
    breadcrumbs: getBreadcrumbs(['Main UI', 'V1'])
  })
}])
...
router.get('/lists/external-movements', [
  ...
  (req, res, next) => {
    ...
    req.renderOptions.breadcrumbs = getBreadcrumbs(['Main UI', 'V1', 'Lists'])
    next()
  },
...
router.get('/visualisations/external-movements-by-:groupField-:chartType', [
  ...
  (req, res, next) => {
    ...
    req.renderOptions = {
      ...req.renderOptions,
      title: 'External movements',
      breadcrumbs: getBreadcrumbs(['Main UI', 'V1', 'Visualisations'])
    }
    next()
  },
