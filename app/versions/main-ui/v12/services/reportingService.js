...
const { pndMetrics } = require('../data/pnd')
...
const filter = (data, filters, dataFormat) => data
  .filter(row => {
    return !Object.keys(filters).find(filter => {
      ...
      return row[filter].toString().toLowerCase() !== filters[filter].toLowerCase()
    })
  })
...
module.exports = {
  ...
  countPnd: (filters, dataFormat) => filter(pndMetrics, filters, dataFormat).length
}
